...
import { blogPosts, formatDisplayDate } from '../../src/data/contentRepository'
  ...
  return (
    <section className="section-card blog-page">
      ...
      <article className="guide-featured featured-article">
        ...
        <small>
          {featured?.readTime} · {featured ? formatDisplayDate(featured.date) : ''}
        </small>
        <Link href={`/blog/${featured?.slug}`} className="inline-link">Read guide</Link>
      </article>
      ...
      <div className="article-grid">
        {rest.map((post) => (
          <article key={post.slug} className="article-card">
            ...
            <Link href={`/blog/${post.slug}`} className="inline-link">Open article</Link>
          </article>
        ))}
      </div>
    </section>
