...
import { blogPosts, comparisons, tools } from '../src/data/contentRepository'
...
export default function HomePage() {
  const firstPost = blogPosts && blogPosts[0]
  return (
    <>
      <section className="hero-card">
        ...
        <div className="hero-actions">
          <Link href="/tools" className="pill-btn dark">Browse all tools</Link>
          <Link href="/blog" className="pill-btn light">Read latest guides</Link>
        </div>
      ...
      <section className="search-card">
        ...
        <div className="search-wrap">
          ...
          <Link href="/tools" className="pill-btn dark small">Search</Link>
        </div>
      ...
      <section className="section-card">
        ...
        <div className="guide-layout">
          <article className="guide-featured">
            ...
            <small>
              {firstPost?.readTime} · {firstPost ? firstPost.date : ''}
            </small>
            <Link href={`/blog/${firstPost?.slug}`} className="inline-link">Read guide</Link>
          </article>
          <div className="guide-list">
            {blogPosts?.slice(1).map((item) => (
              <article key={item.slug}>
                ...
                <Link href={`/blog/${item.slug}`} className="inline-link">Open</Link>
              </article>
            ))}
          </div>
        </div>
        ...
        <div className="pick-grid">
          {featuredTools.map((item) => (
            <article key={item.slug} className="pick-card">
              ...
              <Link href={`/tools/${item.slug}`} className="inline-link">Review</Link>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="section-card compact-section">
        ...
        <div className="compare-inline-grid">
          {comparisons.slice(0, 6).map((item) => (
            <Link key={item.slug} href={`/compare/${item.slug}`} className="compare-inline-card">
